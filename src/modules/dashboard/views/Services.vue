...
export default {
  ...
  data: () => ({
    ...
    dialogRestore: false,
    headers: [
      { text: "Service", value: "name", width: "300" },
  ...
  methods: {
    ...
    initData() {
      setTimeout(() => {
        // get services
        this.getData("dashboard/services").then((res) => {
          this.desserts = res;
        });
    ...
    editItem(item) {
      ...
      this.getData(`dashboard/services/${item.id}`).then((res) => {
    ...
    async save() {
      if (this.editedIndex > -1) {
        ...
        await this.updateData({
          url: `dashboard/services/${this.editedItem.id}`,
          data: data,
        }).then((res) => {

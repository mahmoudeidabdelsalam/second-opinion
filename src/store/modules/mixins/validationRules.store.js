export default {
  ...
  state: () => ({
    ...
    phoneRules: [
      ...
      (v) => (v && v.length == 9) || "الهاتف يجب ان يكون 9 رقم",
    ...
    nationalIdRules: [
      (v) => !!v || "حقل رقم الهوية مطلوب",
      (v) => (v && v.length == 10) || "رقم الهوية يجب ان يكون 10 ارقام",
      (v) => /^\d+$/.test(v) || "رقم الهوية يجب ان يكون ارقام فقط",
    ],
    ...
    jobIdRules: [(v) => !!v || "حقل الرقم الوظيفى مطلوب"],

    // date rules
    dateRules: [(v) => !!v || "حقل التاريخ مطلوب"],
  }),
  ...
  getters: {
    ...
    // job id rules
    jobIdRules: (state) => state.jobIdRules,

    // date rules
    dateRules: (state) => state.dateRules,
  },
};
